...
function cambiarVista(vista) {
  ...
  if (vista === "deudores") cargarDeudores()
...
// ============================================================================
// DEUDORES — LISTA DE CLIENTES CON DEUDA
// ============================================================================

// Cache en memoria de los deudores cargados, para filtrar sin reconsultar.
let deudoresCache = []

/** Carga todos los clientes con deuda > 0 desde Firestore */
async function cargarDeudores() {
  ...
  try {
    ...
    snap.forEach((d) => {
      const data = d.data()
      if ((Number(data.deuda) || 0) > 0) {
        deudoresCache.push({ id: d.id, ...data })
      }
    })
    // Ordenamos de mayor a menor deuda
    deudoresCache.sort((a, b) => (Number(b.deuda) || 0) - (Number(a.deuda) || 0))
    renderizarDeudores()
  } catch (err) {
    console.log("[v0] Error al cargar deudores:", err.message)
    notificar("No se pudo cargar la lista de deudores", "error")
  } finally {
    mostrarLoader(false)
  }
}
...
function renderizarDeudores() {
  ...
  const total = visibles.reduce((acc, c) => acc + (Number(c.deuda) || 0), 0)
  $("deudores-total").textContent = formatearCLP(total)
  $("deudores-count").textContent =
    `${visibles.length} ${visibles.length === 1 ? "deudor" : "deudores"}`

  if (visibles.length === 0) {
    lista.innerHTML = '<p class="muted">Sin deudores por mostrar.</p>'
    return
  }
  ...
  visibles.forEach((c) => {
    ...
    `
    // Al hacer clic, abrimos su ficha en la vista Inicio
...
function init() {
  ...
  $("refresh-deudores").addEventListener("click", cargarDeudores)
  $("filter-deudores").addEventListener("input", renderizarDeudores)
